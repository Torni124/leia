...
function renderPrompt(sourceFile, options = {}) {
    const root = sourceFile.root;
    if (!root) {
        return "";
    }
    const formattedSpec = (0, formatter_1.formatSourceFile)(sourceFile).trimEnd();
...
function renderOutputRequirements(sourceFile) {
...
function renderImplementationGuidance(sourceFile) {
    const targetParts = sourceFile.root?.target?.parts ?? [];
    const [platform, runtime] = targetParts;
    const guidance = [
