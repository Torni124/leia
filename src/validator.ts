import type { FieldEntry, Section, SourceFile } from "./ast";
import { isFieldSection } from "./ast";
...
export function validateSourceFile(
  ...
  if (!root) {
    diagnostics.error(
      "semantic.missing_root",
      "File is missing a valid root declaration.",
      sourceFile.span,
  ...
  if (!root.name) {
    diagnostics.error(
      "semantic.missing_root_name",
      "Root declaration is missing its name.",
      root.span,
      { fixHint: "Provide a name after the root kind." }
    );
  }

  if (!root.target) {
    diagnostics.error(
      "semantic.missing_target",
      "Exactly one `target` declaration is required.",
      root.span,
      { fixHint: "Add a line such as `target react typescript` near the top of the file." }
    );
  }
  ...
  for (const [kind, sections] of sectionsByKind) {
    ...
    if (spec.unique && sections.length > 1) {
      ...
      for (const duplicate of duplicates) {
        diagnostics.error(
          "semantic.duplicate_section",
          `Section \`${kind}\` may only appear once.`,
          duplicate.span,
          {
            relatedInformation: [
              {
  ...
  for (const section of root.sections) {
    ...
    if (
      (section.kind === "rules" || section.kind === "constraints" || section.kind === "tests") &&
      section.content.length === 0
    ) {
      diagnostics.warning(
        "semantic.empty_section",
        `Section \`${section.kind}\` is empty.`,
        section.span,
        { fixHint: `Add at least one item to \`${section.kind}\` or remove the section.` }
      );
    }
  }
  ...
  if (hasConstraints && !hasTests) {
    diagnostics.warning(
      "semantic.constraints_without_tests",
      "Constraints exist without tests to verify them.",
      root.span,
      { fixHint: "Add a `tests:` section to capture acceptance checks for the constraints." }
    );
  }

  if (hasPreferences && !hasRules) {
    diagnostics.warning(
      "semantic.preferences_without_rules",
      "Preferences are defined without a `rules` section.",
      root.span,
      { fixHint: "Add a `rules:` section for hard behavior requirements." }
    );
  }
...
function validateFieldSection(
  ...
  const names = new Map<string, FieldEntry>();
  ...
  for (const field of fields) {
    ...
    if (existing) {
      diagnostics.error(
        ...
        `Duplicate field name \`${field.name.text}\` in section \`${sectionKind}\`.`,
        field.name.span,
        {
          relatedInformation: [
            {
